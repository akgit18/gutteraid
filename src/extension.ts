...
const inputCache: Map<string, Map<string, string>> = new Map(); // matcherId -> inputId -> value
const inputDefinitions: Map<string, TaskInput> = new Map(); // inputId -> TaskInput
...
function watchTaskPatternsFile({
  dirName,
  basePatternsPath,
  localPatternsPath,
  controller,
...
function showInputResetMessage(taskLabel?: string) {
  ...
  if (askEveryTime) {
    vscode.window
      .showInformationMessage<string>(`You do not currently allow caching input choices.`, {modal: true}, 'Enable')
      .then((response) => {
        if (response === 'Enable') {
  ...
  while (queue.length > 0 && !token.isCancellationRequested) {
    ...
    try {
      ...
      const args = script.map((arg) => {
        ...
        processedArg = processedArg.replace(/\$\{input:([^}]+)\}/g, (match, inputId) => inputs.get(inputId) || '');
      ...
      switch (terminalBehavior) {
        ...
        case '':
          ...
          break;
        default:
          const terminalName = terminalBehavior
            ?.replace(/\$\{group:(\d+)\}/g, (match, index) => {
              const groupIndex = parseInt(index) - 1;
              return matchGroups[groupIndex] || '';
            })
            .replace(/\$\{input:([^}]+)\}/g, (match, inputId) => inputs.get(inputId) || '')
...
async function collectInputs(
  matcherId: string,
  argsToUse: string[],
  terminalBehavior: string | undefined,
